...
class ReservoirSamplingBuffer(ExemplarsBuffer):
    ...
    def update(self, strategy: "SupervisedTemplate", no_update=False, full_data=False, **kwargs):
        """Update buffer."""
        if no_update:
            return
        if full_data:
            experience = strategy.experience.dataset
        else:
            experience = strategy.adapted_dataset
        assert experience is not None
        self.update_from_dataset(experience)
...
class ClassBalancedBuffer(BalancedExemplarsBuffer[ReservoirSamplingBuffer]):
    ...
    def update(self, strategy: "SupervisedTemplate", no_update=False, full_data=False, **kwargs):
        ...
        if full_data:
            ...
            assert strategy.experience is not None
        if strategy.adapted_dataset:
            print(f'number of new data: {len(strategy.adapted_dataset)}')
        self.update_from_dataset(strategy.adapted_dataset, strategy)
...
class ParametricBuffer(BalancedExemplarsBuffer):
    ...
    def update(self, strategy: "SupervisedTemplate", no_update=False, full_data=False, **kwargs):
        ...
        if full_data:
            ...
            assert strategy.experience is not None
        new_data: AvalancheDataset = strategy.adapted_dataset
