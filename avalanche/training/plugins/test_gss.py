...
class GSS_greedyPlugin(SupervisedPlugin):
    ...
    def before_training_exp(
        ...
        if self.ext_mem_list_current_index == 0:
            return
        temp_x_tensors = []
        for elem in self.ext_mem_list_x[:self.ext_mem_list_current_index]:
            temp_x_tensors.append(elem.to("cpu"))
        temp_y_tensors = self.ext_mem_list_y[:self.ext_mem_list_current_index].to("cpu")
    ...
    def after_forward(self, strategy, num_workers=0, shuffle=True, no_update=False, **kwargs):
        """
        After every forward this function select sample to fill
        the memory buffer based on cosine similarity
        """
        if no_update or (strategy.clock.train_exp_epochs != (strategy.train_epochs-1)):
            return
