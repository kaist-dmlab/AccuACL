...
from avalanche.benchmarks.utils.data_loader import ReplayDataLoader, _make_data_loader
...
class ReplayPlugin(SupervisedPlugin):
    ...
    def before_training_exp(
        ...
        **kwargs
        ...
        print(f'number of current buffer examples: {len(self.storage_policy.buffer)}')
        print(f'number of labeled examples: {len(strategy.adapted_dataset)}')
        
        if len(self.storage_policy.buffer) == 0:
            # first experience. We don't use the buffer, no need to change
            # the dataloader.
            strategy.dataloader = _make_data_loader(
