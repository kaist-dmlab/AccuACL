...
class FinalStreamAccuracy(AccuracyPluginMetric):
    """
    At the end of the entire stream of experiences, this plugin metric
    reports the average accuracy over all patterns seen in all experiences.
    This metric only works at eval time.
    ...
    def __init__(self):
        """
        Creates an instance of FinalStreamAccuracy metric
        """
        super(FinalStreamAccuracy, self).__init__(
            reset_at="stream", emit_at="stream", mode="train"
        )

    def __str__(self):
        return "Top1_Acc_Stream_Final"
...
def accuracy_metrics(
    *,
    ...
    if final:
        metrics.append(FinalStreamAccuracy())
