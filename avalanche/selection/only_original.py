...
class Original_KMeans(CoresetMethod):
    ...
    def select(self, thresh_sigma=3, **kwargs):
        ...
        df_data['sample_weight'] = F.cross_entropy(F.softmax(torch.tensor(np.array(preds).T), dim=1), torch.tensor(probability), reduction='none').numpy()
