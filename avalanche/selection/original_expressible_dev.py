...
class ACL_Strategy_dev(CoresetMethod):
    def __init__(self, dst_train, strategy, fraction=0.5, mem=None, already_selected=[], selection_method='Margin', index=None, **kwargs):
    ...
    def select(self, thresh_sigma=3, **kwargs):
        ...
        preds = self.map_prototype(df_data['embs'].to_list(), prototypes, thresh_sigma=thresh_sigma)
        df_data['llh'] = np.min(np.array(preds).T, axis=1)
        df_data['prototype'] = np.argmax(np.array(preds).T, axis=1)
        original = df_data['llh']>df_data['llh'].mean()    
        df_original = df_data[original.astype(bool)].copy()
        df_expressible = df_data[~original.astype(bool)].copy()
        print(f'original: {len(df_original)}')
        print(f'expressible: {len(df_expressible)}')
        assert len(df_original) == len(np.where(original)[0])
        ...
        if original_budget==0:
            ...
            expressible_selection = Uncertainty(self.dst_train[np.where(~original)[0].tolist()], self.strategy, fraction=self.fraction, index=df_expressible.index, selection_method=self.selection_method).select()
            ...
            expressible_selection = Uncertainty(self.dst_train[np.where(~original)[0].tolist()], self.strategy, fraction=expressible_budget/len(df_expressible), index=df_expressible.index, selection_method=self.selection_method).select()
            df_original = pd.concat((df_original, df_expressible.loc[expressible_selection]))
            total_selection = k_center_greedy(
                np.vstack(df_original['embs'].to_list()),
                budget=original_budget,
                metric=euclidean_dist,
                index=df_original.index,
                device=self.strategy.device,
                already_selected=expressible_selection
            )
            selection_result = np.append(selection_result, total_selection)
