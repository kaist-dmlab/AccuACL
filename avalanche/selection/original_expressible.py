...
class ACL_Strategy(CoresetMethod):
    ...
    def select(self, thresh_sigma=3, **kwargs):
        ...
        assert set(expressible_selection).issubset(set(selection_result))
        assert set(expressible_selection)&set(selection_result) == set(expressible_selection)
        assert len(set(expressible_selection)&set(selection_result)) == expressible_budget
