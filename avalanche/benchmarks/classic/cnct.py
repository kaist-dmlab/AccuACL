...
def SplitNCT(data_path, num_tasks, train_ratio=0.8):
    train_experiences=[]
    test_experiences=[]
    train_data_path = data_path + '/train'
    test_data_path = data_path + '/test'
    for label, path in enumerate([name for name in os.listdir(train_data_path) if (os.path.isdir(os.path.join(test_data_path, name)) and name!='BACK')]):

        filenames_list = os.listdir(os.path.join(train_data_path, path))
        experience_paths = []
        for name in filenames_list:
            instance_tuple = (os.path.join(train_data_path, path, name), label)
            experience_paths.append(instance_tuple)
        train_experiences.append(experience_paths)
    for label, path in enumerate([name for name in os.listdir(test_data_path) if (os.path.isdir(os.path.join(test_data_path, name)) and name!='BACK')]):
        filenames_list = os.listdir(os.path.join(test_data_path, path))
        experience_paths = []
        for name in filenames_list:
            instance_tuple = (os.path.join(test_data_path, path, name), label)
            experience_paths.append(instance_tuple)
        test_experiences.append(experience_paths)
    num_tasks = 4
