from .ccifar10 import *
from .ccifar100 import *
from .ccub200 import *
from .cfashion_mnist import *
from .cimagenet import *
from .cinaturalist import *
from .cmnist import *
from .comniglot import *
from .core50 import CORe50
from .ctiny_imagenet import *
from .endless_cl_sim import *
from .openloris import *
from .clear import *
from .stream51 import *
from .ex_model import *
from .ccore50 import SplitCORe50
from .cimagenet_r import ImageNet_R
from .cfood100 import SplitFood100
from .cnct import SplitNCT
from .cdomainnet import DomainNet
